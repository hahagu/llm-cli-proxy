  ...
  try {
    ...
    if (provider !== "gemini") {
      throw invalidRequest(
        `Audio translation is not supported for ${provider} models. Use a Gemini model.`,
        "model",
      );
    }
    ...
    return await handleGeminiTranslation(rawModel, {
      audio: audioData,
      prompt,
      temperature,
      responseFormat,
...
async function handleGeminiTranslation(
  ...
  let instruction = "Transcribe the following audio and translate it into English. Return only the English translation, nothing else.";
  if (params.prompt) {
  ...
  const data = (await resp.json()) as GeminiTranslationResponse;
  ...
  if (!text) {
    throw providerError("Gemini returned no translation text");
  }
