  ...
  return new ReadableStream<string>({
    async start(controller) {
      ...
      try {
        ...
        // --- Tool_use tracking (debug only; not emitted to client) ---
        const nativeToolCalls: Array<{ id: string; name: string }> = [];
        let inToolUseBlock = false;
        let turnHadToolUse = false;

        /** Start periodic keepalives during gaps (tool exec / between turns). */
        ...
        for await (const message of sdkQuery) {
          ...
          if (message.type === "stream_event") {
            ...
            if (event.type === "message_start") {
              ...
              // Don't emit separator text between turns — the follow-up
              // turn after a tool call continues the same logical response.
            }

            // --- content_block_start: track tool_use blocks (not emitted) ---
            ...
            if (event.type === "content_block_delta") {
              ...
              if (delta?.type === "input_json_delta" && inToolUseBlock) {
                // Tool args are being streamed — keep the SSE connection alive
                safeEnqueue(": keepalive\n\n");
            ...
            if (event.type === "message_delta") {
              ...
              // If this turn had tool calls, the SDK will now execute the
